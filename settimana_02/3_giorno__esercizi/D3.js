...
while (n < starWarsCharacters.length) {
  crewMass = crewMass + parseInt(starWarsCharacters[n].mass);
  n++;
}
...
switch (true) {
  case (crewMass < 500): messCarico = "Ship is under loaded";
  break;
  case (crewMass = 500): messCarico = "Ship is half loaded";
  break;
  case (crewMass > 700 && crewMass <= 900): messCarico = "Warning: Load is over 700";
  break;
  case (crewMass > 900 && crewMass <= 1000): messCarico = "Critical Load: Over 900";
  break;
  case (crewMass > 1000): messCarico = "DANGER! OVERLOAD ALERT: escape from ship now!";

  console.log(messCarico);
}
